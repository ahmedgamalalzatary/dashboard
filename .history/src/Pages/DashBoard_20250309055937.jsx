...
function DashBoard({ setActivePage }) {
    ...
    // Create a unified navigation function with multiple fallback approaches
    const navigatePage = useCallback((page) => {
        ...
        // Next, try window.setActivePage if it exists
        if (typeof window.setActivePage === 'function') {
            console.log(`Navigation: using window.setActivePage to navigate to ${page}`);
            window.setActivePage(page);
            return true;
        }
        
        // If regular navigation methods aren't available, try URL-based navigation
        console.log(`Standard navigation not available, trying URL navigation to ${page}`);
        return navigateByUrl(page);
    }, [setActivePage, navigateByUrl]);

    // Re-implement the viewAllTransactions function to use navigatePage
    ...
    // Replace duplicated debug effects with a single one that also exposes the navigation function
    useEffect(() => {
        ...
        if (lastAttempt) {
            console.log(`Found failed navigation attempt to: ${lastAttempt}`);
