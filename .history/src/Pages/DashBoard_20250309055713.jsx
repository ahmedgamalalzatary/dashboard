...
function DashBoard({ setActivePage }) {
    ...
    const handleQuickAction = useCallback((action, page, notificationMessage) => {
        if (action === 'navigation') {
            ...
            // Use a timeout to ensure the navigation happens after the notification is shown
            setTimeout(() => {
    ...
    // Create a fallback navigation function when setActivePage is not provided
    const navigatePage = useCallback((page) => {
        if (typeof setActivePage === 'function') {
            console.log(`Navigation: changing to ${page} via props function`);
            setActivePage(page);
        } else {
            // Fallback: try to use window.setActivePage if it exists (global function)
            if (typeof window.setActivePage === 'function') {
                console.log(`Navigation: changing to ${page} via window function`);
                window.setActivePage(page);
            } else {
                console.error(`Navigation failed: cannot navigate to ${page}, setActivePage is not available`);
                // Store the page we tried to navigate to
                localStorage.setItem('lastAttemptedNavigation', page);
                // Show notification
                showTemporaryNotification(`Navigation to ${page} failed. Please use the sidebar menu.`);
            }
        }
    }, [setActivePage, showTemporaryNotification]);
    ...
    useEffect(() => {
        window.debugNavigate = navigatePage;
        return () => {
    ...
    // Debug listener to verify setActivePage is working
    useEffect(() => {
        console.log('Dashboard mounted, setActivePage function available:', typeof setActivePage === 'function');
        
        // If coming back from a failed navigation, show an alert
        const lastAttempt = localStorage.getItem('lastAttemptedNavigation');
        if (lastAttempt) {
            showTemporaryNotification(`Note: Previous attempt to navigate to ${lastAttempt} failed.`);
            localStorage.removeItem('lastAttemptedNavigation');
        }
        
        return () => console.log('Dashboard unmounting');
    }, [setActivePage, showTemporaryNotification]);

    // Re-implement the viewAllTransactions function to use navigatePage instead of setActivePage
    const viewAllTransactions = useCallback(() => {
        ...
        navigatePage('sales');
    }, [navigatePage, showTemporaryNotification]);

    // Alternative navigation approach using URL/location change
    const navigateByUrl = useCallback((page) => {
        try {
            // Show notification first
            showTemporaryNotification(`Navigating to ${page} page`);
            
            console.log(`Attempting direct navigation to: ${page}`);
            
            // First approach: Try window.location
            const baseUrl = window.location.href.split('#')[0];
            const newUrl = baseUrl + '#/' + page;
            
            // Store the target page in localStorage to maintain state across page reloads
            localStorage.setItem('targetPage', page);
            
            // Change URL - this should trigger a route change if using hash router
            window.location.href = newUrl;
            
            // Force a refresh as a fallback if URL change doesn't trigger a router update
            setTimeout(() => {
                // Only refresh if we're still on the same component (navigation didn't work)
                const currentPath = window.location.pathname;
                if (!currentPath.includes(page)) {
                    console.log(`Navigation via URL didn't work, forcing refresh`);
            ...
            showTemporaryNotification(`Navigation error. Please use the sidebar menu.`);
            return false;
        }
    }, [showTemporaryNotification]);
    
    // Enhanced navigation function that tries multiple approaches
    const navigatePage = useCallback((page) => {
        // Try the prop function first
        if (typeof setActivePage === 'function') {
            console.log(`Using setActivePage function to navigate to ${page}`);
            setActivePage(page);
            return;
        }
        
        // If prop function isn't available, try URL-based navigation
        console.log(`setActivePage not available, trying URL navigation to ${page}`);
        navigateByUrl(page);
        
    }, [setActivePage, navigateByUrl]);

    // Replace duplicated debug effects with a single one
    useEffect(() => {
        console.log('Dashboard mounted, attempting to determine navigation capability');
        
        // Check for navigation function
        const hasNavigationFunction = typeof setActivePage === 'function';
        console.log(`setActivePage is a function: ${hasNavigationFunction}`);
        
        // Check for existing localStorage navigation target
        const targetPage = localStorage.getItem('targetPage');
        if (targetPage) {
            console.log(`Found pending navigation to: ${targetPage}`);
            showTemporaryNotification(`Continuing to ${targetPage} page`);
            localStorage.removeItem('targetPage');
            
            // Try to navigate after a brief delay
            setTimeout(() => navigatePage(targetPage), 200);
        }
        
        return () => {
            console.log('Dashboard unmounting');
        };
    }, [setActivePage, navigatePage, showTemporaryNotification]);
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        if (action.action === 'navigation') {
                                            // Show notification and navigate directly
