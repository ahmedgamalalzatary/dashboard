...
function Inventory() {
    ...
    const renderAddStockForm = () => (
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                    ...
                    {inventoryData.map(item => (
                        <option key={item.id} value={item.id}>
                            {item.name} - {t('CurrentStock')}: {item.stock}
                        </option>
                    ))}
                </select>
            </div>
            ...
            <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('Inventory.QuantityToAdd')}</label>
                <input
                    type="number"
                    name="quantity"
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-semibold text-gray-800">{t('Inventory.Management')}</h2>
                <Button 
                    variant="primary"
            ...
            >
                {[
                    ...
                    { title: t('Inventory.OutOfStock'), value: inventoryStats.outOfStockItems, color: 'bg-red-100 text-red-800 border-red-200' },
                ...
                <Card hover className="p-4">
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            <option value="low-stock">{t('Inventory.LowStockItems')}</option>
                            <option value="out-of-stock">{t('Inventory.OutOfStockItems')}</option>
                        </select>
                    </div>
                </Card>
            </div>
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`flex items-center px-2 py-1 rounded-full text-xs ${
                                                ...
                                                {item.status === 'in-stock' ? t('Inventory.InStock') :
                                                 item.status === 'low-stock' ? t('Inventory.LowStock') :
