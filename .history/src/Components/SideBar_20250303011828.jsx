  ...
  return (
    ...
    `}>
      ...
      <nav className={`
        ...
        transition-all duration-300 ease-in-out
        ${isCollapsed ? 'px-1 py-2 space-y-2' : 'p-3 space-y-1.5'}
      `}>
        {sidebarItems.map((item, index) => {
          ...
          return (
            <button
              ...
              title={isCollapsed ? itemText : ''}
              className={`
                flex items-center w-full rounded-lg 
                transition-all duration-300 ease-in-out
                hover:shadow-md
                ${isCollapsed ? 'justify-center p-2.5' : 'py-2.5 px-3'}
                ${!isCollapsed && isRTL ? 'flex-row-reverse justify-start' : ''}
                ${isActive 
