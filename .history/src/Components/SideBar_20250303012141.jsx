    ...
    `}>
      ...
      `}>
        <h1 className={`
          ...
          transition-all duration-300 ease-in-out
          ${effectiveCollapsed ? 'text-lg' : 'text-xl'}
        `}>
          {effectiveCollapsed ? ' ' : t('crmSystem')}
        </h1>
      </div>
      ...
      <nav className={`
        ...
        transition-all duration-300 ease-in-out
        ${effectiveCollapsed ? 'px-1 py-2 space-y-2' : 'p-3 space-y-1.5'}
      `}>
        {sidebarItems.map((item, index) => {
          ...
          return (
            <button
              ...
              title={effectiveCollapsed ? itemText : ''}
              className={`
                flex items-center w-full rounded-lg 
                transition-all duration-300 ease-in-out
                hover:shadow-md
                ${effectiveCollapsed ? 'justify-center p-2.5' : 'py-2.5 px-3'}
                ${!effectiveCollapsed && isRTL ? 'flex-row-reverse justify-start' : ''}
                ${isActive 
